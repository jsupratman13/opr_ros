...
static bool isWhiteGreenEdge(cv::Mat &labeling_image, const int x, const int y)
{
	const unsigned short color_green = 1 << COLOR_GREEN; // color_T::GREEN

	if(x < 1 || x >= labeling_image.cols - 1 || y < 1 || y >= labeling_image.rows - 1) // performance degradation?
		return false;
	const unsigned short right = labeling_image.data[y * labeling_image.cols + (x + 1)];
	const unsigned short left  = labeling_image.data[y * labeling_image.cols + (x - 1)];
	const unsigned short above = labeling_image.data[(y - 1) * labeling_image.cols + x];
	const unsigned short below = labeling_image.data[(y + 1) * labeling_image.cols + x];
	if((right & color_green) || (left & color_green) || (above & color_green) || (below & color_green))
		return true;
	return false;
}
...
void detectFieldEdge(cv::Mat &labeling_image, std::vector<int> &field_edge, std::vector<int> &obstacle_pixel)
{
	...
	giftWrapping(field_edge, obstacle_pixel);
}

void giftWrapping(std::vector<int> &field_edge, std::vector<int> &obstacle_pixel)
{
	const int width = field_edge.size();
...
void detectObstacle(std::vector<int> &field_edge, std::vector<int> &obstacle_pixel, std::vector<object_pos> &objects)
{
	const int width = field_edge.size();
...
void detectWhiteLine(cv::Mat &labeling_image, std::vector<object_pos> &white_line)
{
	...
	for(int h = 1; h < labeling_image.rows - 1; h++) {
		for(int w = 1; w < labeling_image.cols - 1; w++) {
			const unsigned short label = labeling_image.data[h * labeling_image.cols + w];
			if(label & color_white) {
				if(isWhiteGreenEdge(labeling_image, w, h)) {
					object_pos pos(w, h);

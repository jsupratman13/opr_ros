...
DetectorYOLO::DetectorYOLO(const int w, const int h) : ObjectDetector(w, h), object_kind_num(4), score_threshold(object_kind_num, 0.0), yolo_ready(false)
{
...
void DetectorYOLO::getObjects(std::vector<object_pos> &objects, std::vector<object_pos> &white_line)
{
	objects.clear();
	white_line.clear();
 ...
 */
void DetectorYOLO::detectBall(std::vector<object_pos> &objects)
{
	if(!yolo_ready)
		return;
...
void DetectorYOLO::getLabelingImage(cv::Mat &labeling_image)
{
...
void DetectorYOLO::setColorTable(int y, int cb, int cr, int margin, unsigned short object_type)
{
	color_table.setColor(y, cb, cr, object_type, margin);
}

void DetectorYOLO::clearColorTable(int y, int cb, int cr, int margin, unsigned short object_type)
{
	color_table.clearColor(y, cb, cr, object_type, margin);
}

void DetectorYOLO::resetColorTable(unsigned short object_type)
{
	color_table.resetColor(object_type);
}

void DetectorYOLO::loadColorTable(std::string filename)
{
	try {
...
void DetectorYOLO::saveColorTable(std::string filename)
{
	try {
...
bool DetectorYOLO::setupYOLO(std::string config_file, std::string weight_file, std::vector<int> thresholds)
{
...
std::vector<struct BoundingBox> DetectorYOLO::getBallBoundingBox(void)
{
	return ball_box;
}

std::vector<struct BoundingBox> DetectorYOLO::getGoalBoundingBox(void)
{
	return goal_post_box;
}

std::vector<struct BoundingBox> DetectorYOLO::getAlliedRobotBoundingBox(void)
{
	return allied_robot_box;
}

std::vector<struct BoundingBox> DetectorYOLO::getEnemyRobotBoundingBox(void)
{

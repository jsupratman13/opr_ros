...
extern "C" int servo_offset[SERV_NUM];  // オフセット保存用
...
int main(int argc, char* argv[])
{
  ...
  ros::Publisher left_shoulder_pitch_pub =
      nh.advertise<std_msgs::Float64>("/left_shoulder_pitch_controller/command", 1);
  ...
  ros::Publisher right_shoulder_pitch_pub =
      nh.advertise<std_msgs::Float64>("/right_shoulder_pitch_controller/command", 1);
  ros::Publisher right_shoulder_roll_pub =
      nh.advertise<std_msgs::Float64>("/right_shoulder_roll_controller/command", 1);
...
//      printf("%dus\n", diff.total_microseconds());
  ptime = now;
//      printf("cnt:%05d mode:%d%d%d%d%d\n", count_time_l, sq_flag.start, sq_flag.straight, sq_flag.ready, sq_flag.walk,
sq_flag.motion);
*/
  //    }
}
